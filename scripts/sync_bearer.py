#!/usr/bin/env python3
"""Merge ``PR_LEANDNA_DATA_API_BEARER_TOKEN`` from ``.env`` into AWS Secrets Manager.
...
BEARER_TOKEN_KEY = "PR_LEANDNA_DATA_API_BEARER_TOKEN"
...
def merge_bearer_token_from_dotenv(
    ...
    updated: list[str] = []
    if BEARER_TOKEN_KEY not in dotenv:
        return merged, updated
    new_val = dotenv[BEARER_TOKEN_KEY]
    if not str(new_val).strip():
        return merged, updated
    if merged.get(BEARER_TOKEN_KEY) != new_val:
        merged[BEARER_TOKEN_KEY] = new_val
        updated.append(BEARER_TOKEN_KEY)
    return merged, updated
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Sync PR_LEANDNA_DATA_API_BEARER_TOKEN from .env into AWS Secrets Manager.",
    )
    ...
    token = (dotenv.get(BEARER_TOKEN_KEY) or "").strip()
    if not token:
        print(f"No {BEARER_TOKEN_KEY} found in {args.env}", file=sys.stderr)
        return 1
    ...
    if not updated:
        print(f"No changes — secret {secret_id!r} already matches .env for {BEARER_TOKEN_KEY}")
        print(f"  {BEARER_TOKEN_KEY}: {token_fingerprint(token)}")
        return 0

    print(f"Secret: {secret_id}  region: {args.region}")
    print(
        f"  update {BEARER_TOKEN_KEY}: "
        f"{token_fingerprint(str(existing.get(BEARER_TOKEN_KEY) or ''))} → {token_fingerprint(token)}"
    )

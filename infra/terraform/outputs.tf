...
output "secrets_manager_arn" {
  value = aws_secretsmanager_secret.bpo.arn
}

output "ecs_cluster_name" {
  value = aws_ecs_cluster.bpo.name
}
...
output "run_task_engineering_portfolio" {
  ...
  value       = <<-EOT
    aws ecs run-task \
      --cluster ${aws_ecs_cluster.bpo.name} \
      --launch-type FARGATE \
      ...
      --overrides '{"containerOverrides":[{"name":"bpo-decks","command":["engineering-portfolio"]}]}' \

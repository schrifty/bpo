"""On-disk cache for Slack Web API read responses (channel list + channel history)."""

from __future__ import annotations

from typing import Any

from .config import CORTEX_SLACK_CACHE_TTL_SECONDS
...
def cache_get(key: str, ttl_seconds: int | None = None) -> Any | None:
    ttl = CORTEX_SLACK_CACHE_TTL_SECONDS if ttl_seconds is None else max(0, int(ttl_seconds))
...
def clear_slack_cache_for_tests() -> None:

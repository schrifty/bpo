...
def pr_is_ai_assisted(pull: dict[str, Any]) -> bool:
    """True when a PR shows Cursor attribution (trailer, label, or agent author)."""
    if pr_is_ai_automated(pull):
        return True
...
def pr_is_ai_automated(pull: dict[str, Any]) -> bool:
    """True when a PR was generated by an AI agent (author or explicit generated markers)."""
    user = pull.get("user") if isinstance(pull.get("user"), dict) else {}
    login = str(user.get("login") or "").strip().lower()
    ...
    for lab in pull.get("labels") or []:
        ...
        if name and any(needle in name for needle in _AI_AUTOMATED_PR_LABEL_NEEDLES):
            return True
    blob = f"{pull.get('title') or ''}\n{pull.get('body') or ''}".lower()
    return any(marker in blob for marker in _AI_AUTOMATED_PR_MARKERS)
...
def get_ai_assisted_prs_pct(
    *,
    ...
    """% AI-Assisted PRs: merged PRs with Cursor attribution ÷ total merged PRs."""
...
def get_ai_automated_prs_pct(
    *,
    ...
    """% AI-Automated PRs: merged PRs generated by AI ÷ total merged PRs."""

...
def upload_pendo_export_spreadsheet(report: dict[str, Any], title: str, folder_id: str) -> str:
    ...
    Persistent titles must keep a single file id. Shared-drive ``files.delete``
    often fails, so we rewrite the existing workbook instead of creating a twin.
    """
    ...
    if existing:
        ...
        logger.info("Updated Pendo export spreadsheet %s (%s)", existing, title)
        return existing

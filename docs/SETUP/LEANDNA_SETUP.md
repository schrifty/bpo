...
The same mutation guard applies to **classic app API** writes (`set-metric-entry`, `delete-metric-entry` via `src/leandna_app_metrics_client.py`).
...
CLI (from repo root, with `.env` loaded): `get-metrics-app`, `get-my-metrics-app`, `get-metrics-data-app`, `set-metric-entry-app`, `delete-metric-entry-app`, `whoami-app` (see `bin/` wrappers; scripts live under `scripts/` without the `-app` suffix).

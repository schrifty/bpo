...
- Label and value use equal vertical inset (`KPI_METRIC_PAD_V`, 5 pt) from the card edges; value sits in a bottom-anchored line box (not a stretched full-height box).
...
- **symmetric insets:** `KPI_METRIC_PAD_V` (5 pt) from the top edge to the label and from the bottom edge to the value; `KPI_METRIC_PAD_H` (10 pt) horizontal inset
- metric label at `KPI_METRIC_LABEL_PT` (10 pt) in `BLACK`, top-aligned in a compact line box under the top inset
- metric value in a compact line box (≈1.25× value pt height) anchored to the bottom inset — not a full-height box with bottom alignment (that leaves a hollow band between label and value)
...
- **Multi-series combo charts** (`add_combo_chart`, e.g. the engineering **Sprint Velocity** slide: story points per board as bars + a tickets line) follow the same rule — pass `suppress_legend=True` and draw a slide-level legend. Series colors are assigned `BRAND_SERIES_COLORS` in **`bar_series` order first, then `line_series`**, so build the legend entries in that exact order. Keep series labels short (abbreviate long board/team names) so a horizontal legend fits under the plot.

...
LeanDNA **Data API** — REST JSON under the resolved base URL (`resolve_leandna_data_api_base_url()` in `src/config.py`; optional `EXECUTION_ENV` with `ST_*` / `PR_*` — see **[`../SETUP/LEANDNA_SETUP.md`](../SETUP/LEANDNA_SETUP.md)**). **Auth:** API key (`LEANDNA_DATA_API_API_KEY` → Auth session Bearer) — same setup doc. **Optional site scope:** `RequestedSites: <comma-separated site ids>`.
...
| Auth | Preferred `LEANDNA_DATA_API_API_KEY` (Auth session); else `LEANDNA_DATA_API_BEARER_TOKEN` — see [`../SETUP/LEANDNA_SETUP.md`](../SETUP/LEANDNA_SETUP.md) |
| JSON | Responses are JSON unless noted (Data Share returns URL metadata for Parquet) |
| Errors | Typical HTTP 401 without valid Bearer or wrong base URL; field shapes vary by tenant — validate with live calls |
...
| Metric data points | `GET` / `POST` / `DELETE /data/Metric/{metricId}/MetricDataPoint` | Raw result rows (date range on GET/DELETE); **mutations** via `data_api_mutate_json` / tool `leandna_data_api_mutate` (blocked when targeting production unless `CORTEX_ALLOW_PRODUCTION_MUTATIONS=true` — see setup doc) |
